...
/obj/item/implant/nanite_pump/attack_self(mob/user)
	...
	var/cloud_id = input(user, "Установите облако с которого будут скачены программы в имплант. Это можно сделать ОДИН РАЗ (При внедрении в тело при таком раскладе остается возможность перевыбора)", "ID облака", 0, 100, 0) as num|null
