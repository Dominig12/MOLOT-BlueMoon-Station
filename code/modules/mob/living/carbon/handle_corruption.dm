...
/mob/living/carbon/proc/handle_corruption()
	...
	switch(event)
		...
		if("harmies")
			...
			SEND_SIGNAL(src, COMSIG_NEURAL_INTERFACE_WRITE_LOG, "Intent subsystem successfully recalibrated.", "SYSTEM", null, 12)
		if("longdeaf")
			ADD_TRAIT(src, TRAIT_DEAF, CORRUPTED_SYSTEM)
			...
			SEND_SIGNAL(src, COMSIG_NEURAL_INTERFACE_WRITE_LOG, "Hearing subsystem successfully shutdown.", "SYSTEM", null, 12)
		if("longknockdown")
			DefaultCombatKnockdown(50)
			to_chat(src, "<span class='warning'>Significant error in actuator subsystem - Rebooting.</span>")
			SEND_SIGNAL(src, COMSIG_NEURAL_INTERFACE_WRITE_LOG, "Significant error in actuator subsystem - Rebooting.", "SYSTEM", null, 12)
		if("shortlimbdisable")
